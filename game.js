const cell = 20;
const wsUrl = "wss://maze-race-server.onrender.com/ws"; // ← آدرس واقعی سرور
...
// --- کنترل لمسی (بدون دکمه)
cv.addEventListener('touchstart', handleTouch);
cv.addEventListener('touchmove', handleTouch);
...
function connect() {
  ...
  ws.onmessage = (ev) => {
    ...
    if (msg.type === 'roomCreated'){ document.getElementById('status').textContent = `اتاق: ${msg.roomId}`; }
    if (msg.type === 'roomJoined'){ document.getElementById('status').textContent = `ورود به اتاق: ${msg.roomId}`; }
    if (msg.type === 'start'){
      maze = generateMaze(msg.w, msg.h, msg.seed);
      const p1 = maze.start, p2 = {x: maze.start.x, y: maze.start.y+1};
